...
module.exports = function(opts){
    ...
    if (!_.isString(opts.dbtype) || !_.isString(opts.tileRoute)) {
		throw new Error("Must initialise Windwalker with a database type and tileRoute");
	}
    ...
    var render_cache = new RenderCache(1000 * 60 * 15, mml_store);
    ...
    // Tile render.
    app.get(app.tileRoute, function(req, res){
		...
		Step(
            ...
            function handleErrors(err, tile, headers) {
                if (err){
                    ...
                    if(opts.logErrors || opts.logErrorTrace) {
                       console.log("[TILE RENDER ERROR]\n" + err.message);
                         if(opts.logErrorTrace) {
                            console.trace(err)
                        }
                    }  
                } else {

...
module.exports = function(opts){
	...
	app.processTileRoute = function(mapKey, req, res, inParameterParser, inBeforeTileRender, inAfterTileRender) {
		...
		Step(
			...
			function preTileRender(err) {
				...
				if(req.params.datasource == "database" && !req.params.sql) throw new Error("SQL is required after parameter parsing when datasource is 'database'");		
	...
	app.getDefaults = function(params) {
		...
		var default_style = _.template(	
			'#<%= id %>["mapnik::geometry_type"=1] ' + def_style_point +		
			'#<%= id %>["mapnik::geometry_type"=2] ' + def_style_line +		
			'#<%= id %>["mapnik::geometry_type"=3] ' + def_style_poly);

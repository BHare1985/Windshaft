...
module.exports = function(timeout, mml_store){
    ...
    var renderKey = "<%= mapKey %>:<%= datasource %>:<%= pathname %>:<%= dbname %>:<%= format %>:<%= geom_type %>:<%= sql %>:<%= interactivity %>:<%= style %>";
    var baseKey   = "<%= mapKey %>";
    ...
    me.createKey = function(params) {
        ...
        _.defaults(opts, { mapKey:'', datasource: '', pathname: '', dbname:'', format:'', geom_type:'', sql:'', interactivity:'', style:''});
    ...
    me.makeRenderer = function(params, callback){
        ...
        Step(
			function getXmlPathOrString() {
                ...
                mml_store.mml_builder(params).toXML(this);
			},

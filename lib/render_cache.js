...
module.exports = function(timeout, mml_store){
    ...
    var renderKey = "<%= mapKey %>:<%= dbname %>:<%= format %>:<%= geom_type %>:<%= sql %>:<%= interactivity %>:<%= style %>";
    var baseKey   = "<%= mapKey %>";
    ...
    me.createKey = function(params) {
        ...
        _.defaults(opts, {dbname:'', mapKey:'', format:'', geom_type:'', sql:'', interactivity:'', style:''});
    ...
    me.makeRenderer = function(params, callback){
        ...
        Step(
			function initBuilder() {
				mml_builder = mml_store.mml_builder(params).toXML(this);
			},

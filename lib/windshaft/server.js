...
module.exports = function(opts){
    ...
    app.get(app.base_url + '/:z/:x/:y.*', function(req, res){
        ...
        if(!_.isString(req.params.table)) {
            throw new Error("Request must define :table in base_url");
        }
        ...
        var default_style = 		
            '#' + req.params.table + '[mapnik-geometry-type=1]' + def_style_point +		
            '#' + req.params.table + '[mapnik-geometry-type=2]' + def_style_line +		
            '#' + req.params.table + '[mapnik-geometry-type=3]' + def_style_poly;

        
        var default_params = {
            dbtype: opts.dbtype,
            geom_type: opts.geom_type,
            table: req.params.table,
            style: default_style,
            sql: req.params.table
        }
        ...
        Step(
            ...
            function(err) {
                ...
                if(!req.params.sql) delete req.params.sql
                 _.defaults(req.params, default_params);
